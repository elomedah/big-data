...
## Gateway active and reduced modes

The gateway compute profile is also configurable with the same active/reduced
pattern:
...
By default, the active gateway type keeps the profile defaults: `DEV1-S` for
`tiny` and `DEV1-L` for `large`. The reduced gateway type defaults to `DEV1-S`.

## Master active and reduced modes

The Hadoop master compute profile is also configurable:
...
By default, the active master type keeps the profile defaults: `DEV1-S` for
`tiny` and the documented 8 vCPU / 32 GiB master sizing with
...
```text
5 x 600 GB = 3 TB raw HDFS capacity
```
...
Example: increase workers from 600 GB to 800 GB each.

```hcl
cluster_size = "large"
large_worker_data_size_gb = 800
```
...
```bash
# Stop cluster services before any worker, gateway or master compute resize
cd ../ansible
ansible-playbook stop-services.yml

# Before TP, switch workers, gateway and master to active size
cd ../terraform
terraform apply \
  -var='cluster_size=large' \
  -var='worker_mode=active' \
...
# After TP, switch workers, gateway and master to reduced size
terraform apply \
  -var='cluster_size=large' \
  -var='worker_mode=reduced' \
  -var='gateway_mode=reduced' \
