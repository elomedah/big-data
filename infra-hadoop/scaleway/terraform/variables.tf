...
variable "gateway_mode" {
  description = "Gateway compute mode: active for TP sessions, reduced outside TP."
  type        = string
  default     = "active"

  validation {
    condition     = contains(["active", "reduced"], var.gateway_mode)
    error_message = "gateway_mode must be either active or reduced."
  }
}

variable "gateway_active_commercial_type" {
  description = "Scaleway instance type used by the gateway when gateway_mode is active. Defaults to DEV1-S for tiny and DEV1-L for large."
  type        = string
  default     = null
}

variable "gateway_reduced_commercial_type" {
  description = "Scaleway instance type used by the gateway when gateway_mode is reduced."
  type        = string
  default     = null
}

variable "master_mode" {
  description = "Master compute mode: active for TP sessions, reduced outside TP."
  type        = string
  default     = "active"

  validation {
    condition     = contains(["active", "reduced"], var.master_mode)
    error_message = "master_mode must be either active or reduced."
  }
}

variable "master_active_commercial_type" {
  description = "Scaleway instance type used by the Hadoop master when master_mode is active. Defaults to DEV1-S for tiny and BASIC3-X8C-32G for large."
  type        = string
  default     = null
}

variable "master_reduced_commercial_type" {
  description = "Scaleway instance type used by the Hadoop master when master_mode is reduced. Defaults to DEV1-S for tiny and DEV1-L for large."
  type        = string
  default     = null
}
