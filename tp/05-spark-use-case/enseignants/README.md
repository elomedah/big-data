...
1. Pourquoi les équipes déposent-elles leurs fichiers dans des sous-dossiers distincts de `raw` ?
   Réponse indicative : Les sous-dossiers isolent les producteurs, facilitent les droits HDFS, la traçabilité et le retraitement ciblé.
2. Pourquoi conserve-t-on une organisation par année, mois et jour dès la zone brute ?
   Réponse indicative : Le partitionnement temporel permet de retrouver vite une journée, de limiter les lectures et de préparer les traitements incrémentaux.
...
1. Comment identifieriez-vous l’équipe source d’un fichier sans ajouter une colonne dans le CSV ?
   Réponse indicative : On peut l'extraire du chemin HDFS avec `input_file_name()` et une expression régulière.
2. Quels contrôles devriez-vous effectuer au moment du dépôt dans `raw` ?
   Réponse indicative : Présence du fichier, taille non nulle, en-tête attendu, colonnes obligatoires, format de date, séparateur, droits et date de partition cohérente.
...
1. Pourquoi séparer les logs et les référentiels ?
   Réponse indicative : Les logs sont des événements nombreux et variables. Les référentiels décrivent des entités plus stables et évitent la duplication.
2. Que se passe-t-il si un `app_id` existe dans les logs mais pas dans le référentiel ?
   Réponse indicative : La jointure laisse les colonnes enrichies à `null`. La ligne doit être conservée et signalée comme non référencée.
...
1. Pourquoi définit-on un schéma explicite au lieu d’utiliser `inferSchema` ?
   Réponse indicative : Pour garantir des types reproductibles, accélérer la lecture et éviter les mauvaises inférences sur des fichiers hétérogènes.
2. Comment la colonne `source_team` est-elle déduite ?
   Réponse indicative : Elle est extraite du chemin du fichier source avec `input_file_name()`.
...
1. Pourquoi convertir `event_ts` en timestamp ?
   Réponse indicative : Pour permettre les comparaisons temporelles, tris, fenêtres, contrôles et agrégations par date ou heure.
2. Pourquoi créer une colonne `event_date` ?
   Réponse indicative : Elle sert au grain quotidien et au partitionnement des sorties.
3. Quel est l’intérêt des colonnes booléennes `is_error` et `is_warning` ?
   Réponse indicative : Elles simplifient les agrégations par somme et rendent les indicateurs plus lisibles.
...
1. Pourquoi utilise-t-on ici une jointure `left` ?
   Réponse indicative : Pour conserver tous les logs, même si un référentiel est incomplet.
2. Que signifie une ligne avec `app_name` à `null` ?
   Réponse indicative : L'application du log n'a pas été trouvée dans le référentiel.
3. Pourquoi peut-on utiliser `broadcast` sur les référentiels ?
   Réponse indicative : Ils sont petits et peuvent être copiés en mémoire sur les executors.
4. Dans quel cas le `broadcast` deviendrait-il dangereux ?
   Réponse indicative : Si le référentiel devient volumineux, il peut saturer la mémoire du driver ou des executors.
...
1. Pourquoi agréger par `event_date` et `app_id` ?
   Réponse indicative : Ce grain donne un indicateur journalier par application, utile pour l'exploitation et les analyses Hive.
2. Quelle différence faites-vous entre `owner_team` et `source_team` ?
   Réponse indicative : `owner_team` vient du référentiel applicatif. `source_team` vient du producteur ou du chemin de dépôt.
3. Pourquoi le taux d’erreur est-il souvent plus utile que le nombre brut d’erreurs ?
   Réponse indicative : Il permet de comparer des applications avec des volumes très différents.
...
1. Pourquoi partitionner les logs enrichis par `event_date` ?
   Réponse indicative : Pour lire et retraiter seulement les jours nécessaires.
2. Pourquoi ajouter `app_id` dans le partitionnement ?
   Réponse indicative : Pour accélérer les lectures ciblées par application si la cardinalité reste raisonnable.
3. Pourquoi Parquet est-il plus adapté que CSV pour la zone `processed` ?
   Réponse indicative : Parquet conserve les types, compresse mieux et lit seulement les colonnes utiles.
...
1. Où voyez-vous les colonnes de partition dans le schéma ?
   Réponse indicative : Spark les reconstruit depuis les dossiers de partition et les affiche dans `printSchema()`.
2. Pourquoi Spark peut-il éviter de lire certaines partitions ?
   Réponse indicative : Grâce au partition pruning lorsque le filtre porte sur les colonnes de partition.
...
1. Quelle différence faites-vous entre un format ligne et un format colonne ?
   Réponse indicative : Un format ligne stocke les champs d'une ligne ensemble. Un format colonne stocke les valeurs d'une même colonne ensemble.
2. Pourquoi Parquet et ORC sont-ils adaptés aux traitements analytiques ?
   Réponse indicative : Ils sont typés, compressés, colonnes et compatibles avec Spark/Hive.
...
1. Pourquoi ne faut-il pas simplement supprimer les lignes invalides ?
   Réponse indicative : Les supprimer ferait perdre la traçabilité et empêcherait d'analyser la qualité des données reçues.
2. Quelles colonnes ajouteriez-vous pour expliquer la raison du rejet ?
   Réponse indicative : `reject_reason`, `reject_ts`, `source_file`, `source_team`, `process_date`, `pipeline_run_id`.
...
1. Quelle différence faites-vous entre `repartition` et `coalesce` ?
   Réponse indicative : `repartition` redistribue les données avec shuffle. `coalesce` réduit souvent le nombre de partitions avec moins de mouvement.
2. Pourquoi `coalesce(1)` peut-il être dangereux sur de gros volumes ?
   Réponse indicative : Il force une seule tâche d'écriture et crée un goulot d'étranglement.
3. Pourquoi un trop grand nombre de petits fichiers pose-t-il problème dans HDFS ?
   Réponse indicative : Il surcharge le NameNode et ralentit les lectures et la planification des tâches.
...
1. Combien de jobs Spark votre application exécute-t-elle ?
   Réponse indicative : Cela dépend des actions lancées : `show`, `count`, `write`, etc. Chaque action peut déclencher un ou plusieurs jobs.
2. Quelles actions déclenchent ces jobs ?
   Réponse indicative : Les actions comme `show`, `count`, `collect` et les écritures.
3. Où voyez-vous les stages ?
   Réponse indicative : Dans Spark UI ou Spark History Server, onglet `Stages`.
4. Quels indices indiquent qu’une jointure ou une agrégation provoque un shuffle ?
   Réponse indicative : Des stages séparés, des métriques `shuffle read/write`, des opérateurs `Exchange` et parfois des stages réutilisés ou skipped.
Note sur les skipped stages :
`Skipped Stages` n'est pas une erreur. Spark indique qu'il a réutilisé un résultat déjà disponible, par exemple un shuffle matérialisé par une action précédente.
...
1. Comment organiseriez-vous la zone `raw` si dix équipes déposent chaque jour des logs ?
   Réponse indicative : Par producteur, dataset et date : `/raw/<team>/<dataset>/year=YYYY/month=MM/day=DD/`.
2. Comment éviteriez-vous qu’une équipe écrase les fichiers d’une autre équipe ?
   Réponse indicative : Droits HDFS, zones séparées, conventions de nommage, dépôts append-only et contrôles à l'ingestion.
3. Comment géreriez-vous l’arrivée tardive de logs pour une date déjà traitée ?
   Réponse indicative : Retraitement ciblé des partitions, séparation entre date d'événement et date de traitement, versionnement des sorties.
4. Comment distingueriez-vous la date d’événement et la date de traitement ?
   Réponse indicative : La date d'événement vient du log. La date de traitement décrit l'exécution du pipeline.
5. Quelles métadonnées écririez-vous pour chaque exécution Spark ?
   Réponse indicative : Run id, version du code, paramètres, dates traitées, chemins lus, lignes lues, valides, rejetées et écrites.
6. Comment garantiriez-vous qu’un indicateur produit est reproductible ?
   Réponse indicative : Données brutes immuables, référentiels versionnés, paramètres conservés, code versionné et sorties historisées.
7. Quels compromis voyez-vous entre performance, coût de stockage, lisibilité et auditabilité ?
   Réponse indicative : Plus de partitions et d'audit améliore la lisibilité et la traçabilité, mais peut augmenter le stockage et créer des petits fichiers.
...
1. Pourquoi est-il préférable de ne pas mettre tout le code Spark dans un seul fichier ?
   Réponse indicative : Pour améliorer la lisibilité, la maintenance, les tests et le travail en équipe.
2. À quoi sert l’option `--py-files` de `spark-submit` ?
   Réponse indicative : Elle distribue le package Python aux executors Spark.
3. Que faudrait-il ajouter pour transformer cet exemple en projet industrialisable ?
   Réponse indicative : Tests, packaging, configuration par environnement, logs applicatifs, CI/CD, monitoring et gestion des secrets.
